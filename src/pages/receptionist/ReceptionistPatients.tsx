      ...
      <div style={{display:"flex",flexDirection:"column",gap:20,animation:"fadeUp .4s ease both"}}>
        ...
        <div style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:14,overflow:"hidden"}}>
          ...
          {!isLoading&&filtered.map((row:any,i:number)=>(
            <div key={row.id} className="pt-row" onClick={()=>navigate(`/receptionist/patients/${row.id}`)} style={{display:"grid",gridTemplateColumns:"2.2fr 130px 80px 110px 120px 80px 40px",padding:"11px 18px",borderBottom:i<filtered.length-1?`1px solid ${C.border}`:"none",alignItems:"center",transition:"background .1s"}}>
