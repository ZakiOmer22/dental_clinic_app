...
const getPageMeta = (pathname: string, role?: string) => {
    const baseMeta: Record<string, { title: string; description: string }> = {
        // Assistant routes
        "/": { title: "Assistant Dashboard", description: "Today's clinical schedule" },
        "/assistant/appointments/today": { title: "Today's Patients", description: "Patients scheduled for today" },
        "/assistant/patients": { title: "Patients", description: "Manage patient records" },
        "/assistant/patients/:id": { title: "Patient Profile", description: "View patient details" },
        "/assistant/treatments": { title: "Treatments", description: "Clinical notes & procedures" },
        "/assistant/treatments/new": { title: "Start Treatment", description: "Begin a new treatment session" },
        "/assistant/dental-chart": { title: "Dental Chart", description: "Update odontogram & tooth conditions" },
        "/assistant/procedures": { title: "Procedures", description: "Record procedures performed" },
        "/assistant/patient-files": { title: "X-rays & Images", description: "Upload and manage patient images" },
        "/assistant/prescriptions": { title: "Prescriptions", description: "View and dispense medications" },
        "/assistant/lab-orders": { title: "Lab Orders", description: "Track laboratory work" },
        "/assistant/referrals": { title: "Referrals", description: "Manage specialist referrals" },
        "/assistant/consent-forms": { title: "Consent Forms", description: "Signed patient consents" },
        "/assistant/rooms": { title: "Rooms & Chairs", description: "Manage dental rooms" },
        "/assistant/inventory": { title: "Inventory", description: "Check stock levels" },
        "/assistant/medical-history": { title: "Medical History", description: "View patient medical records" },
        "/assistant/support/knowledge": { title: "Knowledge Base", description: "Documentation & resources" },
        "/assistant/support/feedback": { title: "Feedback", description: "Send feedback" },
    };
...
export default function AssistantTopbar() {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        if (searchQuery.trim()) {
            navigate(`/assistant/search?q=${encodeURIComponent(searchQuery)}`);
            setShowSearch(false);
            setSearchQuery("");
        }
    };
