...
import { Star, ThumbsUp, ThumbsDown, MessageSquare, TrendingUp, Users, Heart, Filter, Search, X, CheckCircle2 } from "lucide-react";
...
const GS = `@keyframes fadeUp{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}.fb-card:hover{border-color:${C.tealBorder}!important}`;
...
function Stars({ rating }: { rating: number }) { return <div style={{ display: "flex", gap: 2 }}>{[1, 2, 3, 4, 5].map(s => <Star key={s} size={14} fill={s <= rating ? C.amber : "none"} color={s <= rating ? C.amber : C.border} />)}</div>; }

export function AssistantFeedbackPage() {
    const qc = useQueryClient(); const user = useAuthStore(s => s.user);
    const [search, setSearch] = useState(""); const [typeFilter, setTypeFilter] = useState("all"); const [respondingTo, setRespondingTo] = useState<any>(null); const [response, setResponse] = useState("");
    const { data, isLoading } = useQuery({ queryKey: ["feedback"], queryFn: () => apiGetFeedback() });
    const { data: statsData } = useQuery({ queryKey: ["feedback-stats"], queryFn: () => apiGetFeedbackStats() });
    const feedback: any[] = data?.data ?? data ?? []; const stats: any = statsData ?? {};

    const filtered = useMemo(() => feedback.filter(f => { if (typeFilter !== "all" && f.type !== typeFilter) return false; if (search && !f.comment?.toLowerCase().includes(search.toLowerCase()) && !f.user_name?.toLowerCase().includes(search.toLowerCase())) return false; return true; }), [feedback, typeFilter, search]);

    const respondMut = useMutation({ mutationFn: ({ id, response }: any) => apiRespondToFeedback(id, { response, respondedBy: user?.id }), onSuccess: () => { toast.success("Response sent"); qc.invalidateQueries({ queryKey: ["feedback"] }); setRespondingTo(null); setResponse(""); } });

    const avgRating = (stats.totalRatings > 0 ? stats.sumRatings / stats.totalRatings : 0).toFixed(1);
    const formatTime = (ts: string) => { const d = new Date(ts); return d.toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" }); };
    ...
    return (<>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20, animation: "fadeUp .4s ease both" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
                <div><h1 style={{ fontSize: 21, fontWeight: 700, color: C.text, letterSpacing: "-.02em" }}>Feedback</h1><p style={{ fontSize: 13, color: C.faint, marginTop: 2 }}>{feedback.length} submissions · {feedback.filter(f => !f.responded_at).length} pending response</p></div>
            </div>
            ...
            <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 14, padding: "18px 20px" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {[5, 4, 3, 2, 1].map(r => {
                        const count = feedback.filter(f => f.rating === r).length; const pct = feedback.length > 0 ? Math.round((count / feedback.length) * 100) : 0;
                        return <div key={r} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 2, width: 60 }}><span style={{ fontSize: 12, fontWeight: 600, color: C.text }}>{r}</span><Star size={12} fill={C.amber} color={C.amber} /></div>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                ...
                <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                    {[{ value: "all", label: "All Types" }, ...TYPES.map(t => ({ value: t, label: TYPE_CFG[t]?.label ?? t }))].map(t => { const active = typeFilter === t.value; const cfg = TYPE_CFG[t.value]; return <button key={t.value} onClick={() => setTypeFilter(t.value)} style={{ padding: "4px 10px", borderRadius: 8, fontSize: 11, fontWeight: 600, border: `1px solid ${active && cfg ? cfg.color + "40" : C.border}`, background: active && cfg ? cfg.bg : C.bg, color: active && cfg ? cfg.color : C.muted, cursor: "pointer", fontFamily: "inherit" }}>{t.label}</button>; })}
                </div>
            </div>

            {/* Feedback cards */}
            {isLoading ? <div style={{ padding: "40px 18px", textAlign: "center" }}><div style={{ width: 22, height: 22, borderRadius: "50%", border: `2px solid ${C.border}`, borderTopColor: C.teal, animation: "spin .7s linear infinite", margin: "0 auto 8px" }} /><p style={{ fontSize: 13, color: C.faint }}>Loading…</p></div> : filtered.length === 0 ? <div style={{ padding: "48px 18px", textAlign: "center" }}><MessageSquare size={28} color={C.border} style={{ margin: "0 auto 10px", display: "block" }} /><p style={{ fontSize: 13, color: C.faint }}>No feedback found</p></div> :
                <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 14 }}>
                    {filtered.map(fb => {
                        const cfg = TYPE_CFG[fb.type] ?? TYPE_CFG.review; const Icon = cfg.icon;
                        return <div key={fb.id} className="fb-card" style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 14, padding: "16px 18px", transition: "all .15s" }}>
                            <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 10 }}>
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                                        ...
                                        <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 7px", borderRadius: 100, background: cfg.bg, color: cfg.color, border: `1px solid ${cfg.color}40` }}>{cfg.label}</span>
                                    </div>
                            ...
                            {fb.comment && <p style={{ fontSize: 13, color: C.text, lineHeight: 1.6, marginBottom: 10 }}>{fb.comment}</p>}
                            {fb.response ? <div style={{ background: C.tealBg, border: `1px solid ${C.tealBorder}`, borderRadius: 8, padding: "10px 12px", marginBottom: 8 }}>
                                <p style={{ fontSize: 11, fontWeight: 700, color: C.tealText, marginBottom: 4 }}>Response from {fb.responded_by_name}</p>
                                <p style={{ fontSize: 12, color: C.text, lineHeight: 1.5 }}>{fb.response}</p>
                            </div> : respondingTo?.id === fb.id ? <div style={{ marginTop: 8 }}>
                                <textarea value={response} onChange={e => setResponse(e.target.value)} rows={3} placeholder="Write your response…" className="inp" style={{ ...IS, height: "auto", padding: "8px 12px", resize: "none", lineHeight: 1.5, marginBottom: 8 }} />
                                <div style={{ display: "flex", gap: 6 }}>
                                    <button onClick={() => { setRespondingTo(null); setResponse(""); }} style={{ flex: 1, padding: "6px 12px", borderRadius: 6, border: `1px solid ${C.border}`, background: C.bg, fontSize: 12, fontWeight: 500, color: C.muted, cursor: "pointer", fontFamily: "inherit" }}>Cancel</button>
                                    <button onClick={() => { if (response.trim()) respondMut.mutate({ id: fb.id, response: response.trim() }); }} disabled={!response.trim() || respondMut.isPending} style={{ flex: 1, padding: "6px 12px", borderRadius: 6, background: !response.trim() || respondMut.isPending ? "#9ab5ae" : C.teal, border: "none", color: "white", fontSize: 12, fontWeight: 600, cursor: !response.trim() || respondMut.isPending ? "not-allowed" : "pointer", fontFamily: "inherit" }}>Send Response</button>
                                </div>
                            </div> : <button onClick={() => setRespondingTo(fb)} style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: `1px solid ${C.tealBorder}`, background: C.tealBg, fontSize: 12, fontWeight: 600, color: C.tealText, cursor: "pointer", fontFamily: "inherit", marginTop: 8 }}>Respond to Feedback</button>}
                        </div>;
                    })}
                </div>}
        </div>
