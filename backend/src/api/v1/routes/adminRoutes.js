...
router.get('/clinics/:id/treatments', auth, adminOnly, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(`
      SELECT t.id, t.treatment_date as date, t.status, t.notes,
             p.full_name as patient_name,
             u.full_name as doctor_name,
             tp.name as procedure
      FROM treatments t
      LEFT JOIN patients p ON t.patient_id = p.id
      LEFT JOIN users u ON t.doctor_id = u.id
      LEFT JOIN treatment_procedures tp ON t.procedure_id = tp.id
      WHERE t.clinic_id = $1
...
router.get('/clinics/:id/prescriptions', auth, adminOnly, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(`
      SELECT pr.id, pr.medication_name as medication, pr.dosage, 
             pr.frequency, pr.status, pr.created_at as prescribed,
             p.full_name as patient_name,
             u.full_name as doctor_name
...
router.get('/clinics/:id/inventory', auth, adminOnly, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(`
      SELECT id, name as item_name, category, quantity_in_stock as stock,
             minimum_stock_level as min_level, unit, status
...
router.get('/audit-logs', auth, adminOnly, async (req, res) => {
  try {
    if (!isSuperAdmin(req.user)) {
      const { rows } = await pool.query(`
        SELECT 
          id, actor, action, target, ip, 
          TO_CHAR(created_at, 'Mon DD · HH24:MI') as time,
          category
        FROM audit_logs 
        WHERE clinic_id = $1
        ORDER BY created_at DESC
